...
assign
  ...
  config-item.itemType    = "Startup"
  std-ch                  = replace(config-item.itemLabel, " ", "")
  .
  
&if defined(loadProc) = 0 &then
config-item.itemLoadProc = "Load" + std-ch.
&else
config-item.itemLoadProc = "{&loadProc}".
&endif
  
&if defined(getProc) = 0 &then
config-item.itemGetProc = "GetLoad" + std-ch.
&else
config-item.itemGetProc = "{&getProc}".
&endif
  
&if defined(setProc) = 0 &then
config-item.itemSetProc = "SetLoad" + std-ch.
&else
config-item.itemSetProc = "{&setProc}".
&endif
  
&if defined(modProc) = 0 &then
config-item.itemModProc = substring(std-ch, 1, length(std-ch) - 1) + "DataChanged".
&else
config-item.itemModProc = "{&modProc}".
&endif
